from ignore import database_link
...
# Loop through the current folder, and delete all existing the csv files
...
def get_new_csv():
	...
	open("prijslijst-scanners.csv", "wb").write(response.content)
	return pd.read_csv("prijslijst-scanners.csv")
...
def get_barcode_scanner():
	...
	return usb.util.find_descriptor(intf, custom_match = lambda e: usb.util.endpoint_direction(e.bEndpointAddress) == usb.util.ENDPOINT_IN)
...
def remove_items(byteList, item):
    byteList = [i for i in byteList if i != item]
    return byteList

def binary_to_ASCII(byteList):
	shift = True if byteList[0] == 2 else False
		
	remove_items(byteList, 0)
	remove_items(byteList, 2)
		
	if len(byteList) == 0:
		return ''
	
	return CONV_TABLE[byteList[0]][1] if shift == True else CONV_TABLE[byteList[0]][0]
...
def data_to_barcode(data, barcode = ""):
	for index in range(int(len(data)/8)):
		barcode += binary_to_ASCII(data[index*8 : index*8+8])
	
	return barcode.rstrip('\n')

def format_price(price):
    if "," in price: # 1,99 -> 1,99
        return price
    
    if "." in price: # 1.99 -> 1,99
        return price.replace(".", ",")
    
    return price[0:len(price)-2] +","+price[len(price)-2:len(price)] # 199 -> 1,99


def main():
	try:
		barcode = data_to_barcode(list(EP.read(1000)))
		...
		else: # if not -> display "product not found"
